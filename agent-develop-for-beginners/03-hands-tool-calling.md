...
> **⚠️ 常见错误示范：缺少类型注解和 Docstring 的危害**
> 
> ```python
> # ❌ 错误写法：没有类型注解和文档说明
...
> ```
> 
> 如果没有写 Docstring 和类型注解，框架生成的 Tool Schema 将非常模糊（甚至报错），大模型可能出现以下问题：
> - 不知道 `location` 应该传城市名还是城市代码，随意填 `"SH"` 或 `"China"`
> - 不清楚这个函数的用途，在不该调用时误触
> 
> **规范的 Docstring 和类型注解，是大模型正确决策调用工具的唯一依据。**
...
在真实业务中，工具不会每次都乖乖成功返回数据。网络超时、API 限流、返回格式不符——这些都是家常便饭。**一个未经任何错误保护的工具调用，往往会让整个 Agent 任务链崩溃在最关键的一步。**
...
def safe_tool_call(tool_fn, *args, retries=3, fallback=None, **kwargs):
    """
    一个通用的工具调用包装器，支持自动重试和降级 Fallback。
    """
    for attempt in range(retries):
        try:
            ...
            if not result or result == "{}":
                print(f"[Warn] 工具返回空结果，第 {attempt+1} 次重试...")
                time.sleep(1)
                continue
            ...
            # 策略2：网络超时，使用指数退避重试
            print(f"[Error] 工具调用超时（第 {attempt+1} 次），正在重试...")
            time.sleep(2 ** attempt)  # 退避：1s → 2s → 4s

        except Exception as e:
            # 策略3：不可恢复的异常，立即停止
            print(f"[Fatal] 工具调用出现致命错误：{e}，停止重试。")
            break

    # 重试耗尽后，使用降级方案（Fallback）
    if fallback is not None:
        print("[Fallback] 使用降级方案兜底...")
        return fallback

    # 若连 Fallback 都没有，返回结构化错误信息让 Brain 知情
    return '{"error": "工具调用多次失败，请尝试其他方案或通知用户。"}'


# 使用方式：
# result = safe_tool_call(get_weather, "巴黎", fallback='{"condition": "未知"}')
```

> 💡 **关键原则**：工具调用失败时，不要让 Agent 静默崩溃。**应将错误信息结构化地返回给 Brain（LLM）**，让它决定是换一种工具、修改参数重试还是告知用户此路不通，而不是无限等待或者抛出未捕获的异常。
